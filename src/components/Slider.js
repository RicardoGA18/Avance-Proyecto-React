...
export default function(){
  let [numPhoto, setPhoto] = useState(1) 

  return (
    <div className="Slider">
      <img src={`/assets/photos/slider-${numPhoto}.jpg`}/>
      <div className="Slider__Left" onClick={()=>{
        if(numPhoto <= 1){
          return setPhoto( numPhoto = 3 )
        }else{
          return setPhoto( numPhoto = numPhoto -1 )
        }
      }}>
      ...
      <div className="Slider__Right" onClick={()=>{
        if(numPhoto >= 3){
          return setPhoto( numPhoto = 1 )
        }else{
          return setPhoto(numPhoto = numPhoto + 1)
        }
      }}>
